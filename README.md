...
<div align="center">
  <img src="https://latex.codecogs.com/svg.latex?\dpi{150}&space;\large&space;\text{Toxicity\_Index}&space;=&space;\sum_{i=1}^{n}&space;\left(W_{cat}^{(i)}&space;\times&space;S_{sev}^{(i)}\right)" title="Toxicity_Index = Sum(W_cat * S_sev)" />
</div>

### Обозначения:

*   **$W_{cat}$ (Вес категории):** Значимость риска (диапазон **5–10** баллов).
